...
### ⏪ 5. Status Bar 1-Click Version Rollback & Switch
* **Safety Rollback Guard**: If a newly updated core introduces bugs or schema mismatches, easily roll back to any previously stable version directly from the macOS top status bar menu.
* **Automatic Database Snapshot**: Automatically creates a timestamped database backup (`narrafork.db.bak_switch_*`) prior to switching versions, ensuring complete data security.
* **Import & Archive Manager**: Easily import external historical binaries or open the `~/.narrafork/versions/` archive directory in Finder with one click.

### 💾 6. Clean Migration Database & Zero Data Leakage
...
### 🎨 7. iOS Continuous Squircle & Retina DMG Installer
